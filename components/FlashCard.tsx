...
export const FlashCard: React.FC<FlashCardProps> = ({ 
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-xl p-8 max-w-md w-full mx-auto transform transition-all hover:scale-[1.02] border-4 border-sky-100 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center">
        {/* Character Display */}
        <div className="text-[10rem] leading-none hanzi-font text-sky-600 mb-6 drop-shadow-sm select-none">
          {character}
        </div>
        ...
        {isLoading ? (
          <div className="h-40 flex flex-col items-center justify-center space-y-4">
            <LoadingSpinner />
            <p className="text-slate-400 text-sm animate-pulse">Loading details...</p>
          </div>
        ) : details ? (
          <div className="w-full text-center space-y-6 animate-fade-in-up">
            
            {/* Pinyin */}
            <div className="text-5xl font-bold text-slate-700">{details.pinyin}</div>
            
            {/* Audio Button */}
            <button 
              onClick={() => onPlayAudio(character)}
              disabled={isAudioLoading}
              className={`group flex items-center justify-center space-x-2 mx-auto bg-sky-100 hover:bg-sky-200 text-sky-700 px-6 py-3 rounded-full transition-all active:scale-95 ${isAudioLoading ? 'opacity-70' : ''}`}
            >
              {isAudioLoading ? (
              ...
              <span className="text-lg font-bold">听 (Listen)</span>
            </button>

            {/* Sentence Card */}
            <div className="bg-slate-50 rounded-2xl p-4 mt-6 border border-slate-100">
               <p className="text-2xl text-slate-700 mb-3 font-medium">{details.sentence}</p>
               <button 
                onClick={() => onPlayAudio(details.sentence)}
                className="text-sm text-sky-500 font-semibold hover:text-sky-600 uppercase tracking-wide flex items-center justify-center gap-1 mx-auto"
               >
